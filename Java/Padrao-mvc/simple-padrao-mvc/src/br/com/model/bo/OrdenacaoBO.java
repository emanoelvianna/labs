...
import br.com.model.dao.OrdenacaoDAO;
import br.com.model.dao.PassosDAO;
...
public class OrdenacaoBO {
	...
	public Ordenacao bubbleSort(int numero) {
		try {
			// Transforma em String para fazer as trocas consideranco caracter
			// por caracter
			// Converto o numero do tipo Int para String e depois gero um array
			// de chars.
			char[] digitos = String.valueOf(numero).toCharArray();
			// Nosso 'balde' intermediario entre as trocas, variavel auxiliar.
			char aux;
			// Outra auxiliar que serve para armazenar o numero antes da
			// modificacao para
			// se criar o 'Passo'
			char[] antes;
			// Variavel que sera incrementada a cada troca para contar quantas
			// trocas houve
			int qtdeTrocas = 0;
			...
			// Sera percorrido todos os numeros de acordo com o tamanho da
			// sequencia
			for (int i = 0; i < digitos.length; i++) {
				if (!continua) { // Verificando se foram feitas trocas no ultimo
									// ciclo, se nao foram, indica que ja esta
									// ordenado
					break; // Interrompe o algoritmo
				}
				// Descrevendo o passo
				passos.add(new Passos(null, null,
						"Inicio da verificacao numero ".concat(String.valueOf(i)).concat("\n------------\n")));
				continua = false;
				...
				for (int j = 0; j < digitos.length - 1; j++) {
					if (digitos[j] > digitos[j + 1]) {
						...
						qtdeTrocas++;
						// Descrevendo o passo
						passos.add(new Passos(new String(antes), new String(digitos),
								"Trocou-se o digito ".concat(String.valueOf(digitos[j + 1])).concat(" pelo ")
										.concat(String.valueOf(digitos[j]))));
						continua = true;
					} else {
						passos.add(
								new Passos(new String(digitos), new String(digitos),
										"Nao houve troca pois o numero ".concat(String.valueOf(digitos[j]))
												.concat(" ja eh menor/igual que ")
